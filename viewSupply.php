<?php
	...
	$id=$_GET["id"];
	$q="select * from supplies where id='$id'";
	$q_run=mysqli_query($con,$q);
	$row=mysqli_fetch_assoc($q_run);
?>
<html>
	<head>
		<title><?php echo $row["item"]; ?>|Product Detail</title>
					...
					<div class="detailContent">
						<table style="border-spacing:0 25px;">
							...
							<tr class="rowData">
								...
								<td class="data"><?php echo $row['item']; ?></td>
							</tr>
							<tr class="rowData">
						...
						<?php
							...
							if($row["qty"] > 0){
								$q2="select * from cart where supplyID='$id' and customerID='$cid'";
						...
						<a href="cart.php" class="btn" style="text-decoration:none;padding:12px;">BUY FROM CART</a>
								...
								?>
								<form method="post" action="updateSupplyCart.php">
									...
									<?php }else{?>
						<form method="post" action="addSupplyCartParse.php">
						...
						<form method="post" action="buySupply.php" style="margin-top:-56px;margin-left:160px;">
							<input type="hidden" name="id" value="<?php echo $id; ?>">
