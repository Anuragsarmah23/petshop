<?php
	...
	if($erroR==1){
		...
		$breed1=mysqli_real_escape_string($con,$_POST['breed']);
		$breed=strtolower($breed1);
		...
		if($qty <= 0){
			...
			$q1="select * from pets where type='$type' and breed='$breed'";
			$q1_run=mysqli_query($con,$q1);
			$rows=mysqli_num_rows($q1_run);
			if($rows > 0){
				echo '<script>alert("Pet exist.. update the quantity of the pet.")</script>';
				echo '<script>location.href="products.php";</script>';
			}	
			else{	
				if (move_uploaded_file($_FILES["File"]["tmp_name"], $targetDir)){
					$q="insert into pets(type,breed,price,qty,discount,pic) values('$type','$breed','$price','$qty','$discount','$fileName')";
