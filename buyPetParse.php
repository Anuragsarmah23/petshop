<?php
	...
	$price=$_SESSION["petPriceP"];	
	$no=rand(1000,9999);
	$tid=$customerID.$no;
	$q="insert into transaction(id,customerID,petID,totalAmount,status) values('$tid','$customerID','$id','$price','pending')";
	$q_run=mysqli_query($con,$q);
	if($q_run){
		$q2="select * from pets where id='$id'";
		$q2_run=mysqli_query($con,$q2);
		$row2=mysqli_fetch_assoc($q2_run);
		$quantity=$row2["qty"];
		$Nquantity=$quantity-1;
		$q3="update pets set qty='$Nquantity' where id='$id'";
...
?>		
		<script>var petID="<?php echo $id ;?>";</script>
